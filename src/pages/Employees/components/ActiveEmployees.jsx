...
import ChangePosition from "./Modals/ChangePosition";

const ActiveEmployees = ({ showListview }) => {
  const dispatch = useDispatch();
  const modals = useSelector(selectModalsSlice);
  const changePosition = modals.CHANGE_position;

  const viewDetails = modals.VIEW_details;
  const deactivateEmployee = modals.DEACTIVATE_employee;
  const permissions = modals.PERMISSIONS;
  ...
  const openPermissionsModal = () => {
    dispatch(openModalByName("PERMISSIONS"));
  };
  ...
  return (
    <div className="mt-8 bg-white dark:bg-deskit-black-400 dark:text-white shadow-lg rounded-xl min-h-[800px] overflow-hidden mb-8 p-8">
      <div>
        <p className="mb-8 font-bold">All Employees</p>
        {showListview ? (
          <div>
            <Table
              pagination={true}
      ...
      {permissions && <Permissions isOpen={permissions} />}
      {changePosition && <ChangePosition isOpen={changePosition} />}
    </div>
  );
};
